...
# ------------------------------------------------------------------ sql setup
# ملفات SQL حقيقية في `supabase/` — تُنسخ إلى SQL Editor كما هي (أسهل من تضمينها
# في الكود، وتُراجع كـSQL لا كنص داخل Python).
SQL_DIR = os.path.join(BASE, "supabase")


def sql_files() -> list[str]:
    try:
        return sorted(name for name in os.listdir(SQL_DIR) if name.endswith(".sql"))
    except OSError:
        return []


def read_sql(name: str) -> str:
    """يقرأ ملف SQL من `supabase/`. يعيد نصًا فارغًا إن لم يوجد."""
    try:
...
SETUP_SQL = read_sql("01_state_snapshots.sql")
...
def main(argv) -> int:
    if "--help" in argv or "-h" in argv:
        ...
        return 0
    if "--sql" in argv:
        # --sql            ⇒ مخطط النسخ الاحتياطي (01)
        # --sql tasks      ⇒ مرآة المهام (02)   |  --sql all ⇒ الكل بالترتيب
        idx = argv.index("--sql")
        which = argv[idx + 1] if idx + 1 < len(argv) and not argv[idx + 1].startswith("-") else "snapshots"
        wanted = {"snapshots": ["01_state_snapshots.sql"],
                  "tasks": ["02_tasks_mirror.sql"],
                  "all": sql_files()}.get(which)
        if not wanted:
            available = ", ".join(sql_files()) or "لا ملفات"
            print(f"❌ غير معروف: {which}\n   المتاح: snapshots · tasks · all   ({available})")
            return 2
        chunks = []
        for name in wanted:
            text = read_sql(name)
            if not text:
                print(f"❌ ملف SQL مفقود: supabase/{name}")
                return 2
            chunks.append(f"-- ############ supabase/{name} ############\n{text.rstrip()}")
        print("\n\n".join(chunks))
        return 0
    if "--live" in argv:
        cfg = load_config()
