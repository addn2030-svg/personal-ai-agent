...
### 🚨 إن لصقت المفتاح السري في محادثة بالخطأ (أو في أي مكان)

القاعدة (1) أعلاه ليست شكلية. المفتاح السري يتجاوز RLS ويقرأ ويكتب كل صف — ومتى
ظهر في محادثة أو سجل أو لقطة شاشة فقد صار مكشوفًا، **ولا يكفي حذف الرسالة**.

الترتيب الصحيح:

1. **أنشئ مفتاحًا سريًا جديدًا**: Supabase → **Settings → API Keys** →
   المفتاح السري → **Rotate / Create new secret key**.
2. **حدّث الخادم**: Render → **Environment** → `SUPABASE_SERVICE_ROLE_KEY` = الجديد
   → أعد النشر.
3. **أبطل القديم**: بعد التأكد أن الخادم يعمل بالمفتاح الجديد.
4. **راجع الأثر**: Supabase → **Logs → API** — ابحث عن أي طلب لم تصدره أنت
   خلال الفترة بين الكشف والإبطال.

**المفتاح العام (`sb_publishable_…`) ليس في هذه القائمة**: هو مخصَّص للنشر في
المتصفح، وRLS هو ما يحمي البيانات منه — لا سرّيته. لذلك لا حاجة لتدويره.

ولمعرفة حالة المشروع بلا أي مفتاح سري: `python3 -m connectors.supabase_probe`
(انظر `docs/git-data-to-sql.md`). الأمر نفسه ينطبق على مفاتيح Gemini/Kimi/Telegram:
أي مفتاح يظهر في محادثة يُعتبر مكشوفًا ويُدوَّر.
